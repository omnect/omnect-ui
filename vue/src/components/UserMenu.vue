	...
	<Menu v-model="menu" :open-on-hover="false">
		<template v-slot:activator="{ props }">
			<picture data-cy="user-menu" class="h-8 w-8 " v-bind="props">
				<source id="s1" :srcset="`https://ui-avatars.com/api/?name=ui&background=0D8ABC&color=fff&rounded=true`" />
		...
		<v-card title="omnect-ui" subtitle="User">
			<v-card-text class="mt-2">
				<div class="flex justify-space-between items-center">
					<v-btn type="button" text="logout" prepend-icon="mdi-logout" variant="text" color="default" @click="logOut">
					</v-btn>
				</div>
			</v-card-text>
		</v-card>
