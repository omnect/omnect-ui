...
impl Api {
    ...
    pub async fn load_update(body: web::Json<LoadUpdate>, api: web::Data<Api>) -> impl Responder {
        debug!("load_update() called with path {body:?}");

        match api.ods_client.load_update(body.into_inner()).await {
            Ok(_) => HttpResponse::Ok().finish(),
            Err(e) => {
