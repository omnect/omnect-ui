...
impl OmnectDeviceServiceClient {
    // ToDo adapt if load_update is changed in ods
    ...
    pub async fn reboot(&self) -> Result<()> {
        self.post_with_empty_body("/reboot/v1").await
    }

    pub async fn reload_network(&self) -> Result<()> {
        self.post_with_empty_body("/reload-network/v1").await
    }

    pub async fn load_update(&self, load_update: LoadUpdate) -> Result<()> {
        self.post_with_json_body(
            concat!("/fwupdate/load/v1/", env!("CARGO_PKG_NAME")),
            load_update,
        )
        .await
    }
    ...
    async fn register_publish_endpoint(&self) -> Result<()> {
        ...
        self.post_with_json_body("/publish-endpoint/v1", body).await
    }

    async fn post_with_empty_body(&self, path: &str) -> Result<()> {
        self.socket_client
    ...
    async fn post_with_json_body(&self, path: &str, body: impl Serialize) -> Result<()> {
        self.socket_client
