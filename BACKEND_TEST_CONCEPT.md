...
The backend currently has **40 unit/integration tests** organized as follows:
...
**Total: 40 tests** (32 unit tests, 5 integration tests, 3 http_client integration tests)
...
**Not Tested:**
- API handlers (`api.rs`)
- Network configuration service (`network.rs`)
...
- [ ] Test `set_network_config` with valid DHCP config
- [ ] Test `set_network_config` with valid static IP config
- [ ] Test validation failure for invalid netmask
- [ ] Test validation failure for empty interface name
- [ ] Test rollback file creation/cancellation
- [ ] Test `rollback_exists()` and `cancel_rollback()`
- [ ] Test network config file generation (INI format)
